        ...
        <div class="px-6 py-4 bg-gray-50 border-b">
            <div class="flex items-center space-x-4">
                <div class="flex-shrink-0">
                    ...
                    @else
                        <div class="h-16 w-16 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center border-4 border-white shadow-lg">
                            <span class="text-white font-bold text-lg">{{ strtoupper(substr($karyawan->full_name, 0, 2)) }}</span>
                        </div>
                    @endif
                </div>
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ $karyawan->full_name }}</h3>
                    <p class="text-gray-600">{{ $karyawan->position }} - {{ $karyawan->department->name ?? 'Belum ada department' }}</p>
...
@endif
        ...
        <!-- Form -->
        <form action="{{ route('admin.karyawan.update', $karyawan->karyawan_id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div id="personal-content" class="tab-content">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $karyawan->full_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" id="birth_date" name="birth_date" value="{{ old('birth_date', $karyawan->birth_date ? $karyawan->birth_date->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="date" id="hire_date" name="hire_date" value="{{ old('hire_date', $karyawan->hire_date ? $karyawan->hire_date->format('Y-m-d') : '') }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <select id="department_id" name="department_id" required
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->department_id }}" {{ old('department_id', $karyawan->department_id) == $department->department_id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="employment_status" name="employment_status" required
                            ...
                            <option value="active" {{ old('employment_status', $karyawan->employment_status) == 'active' ? 'selected' : '' }}>active</option>
                            <option value="inactive" {{ old('employment_status', $karyawan->employment_status) == 'inactive' ? 'selected' : '' }}>inactive</option>
                            <option value="terminated" {{ old('employment_status', $karyawan->employment_status) == 'terminated' ? 'selected' : '' }}>terminated</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center space-x-6">
                            <div class="shrink-0">
                                <img id="photo-preview" class="h-20 w-20 object-cover rounded-full border-4 border-gray-300"
                                     src="{{ $karyawan->photo ? asset('storage/' . $karyawan->photo) : 'https://via.placeholder.com/80/e5e7eb/6b7280?text=Foto' }}"
                                     alt="Preview">
                            </div>
                            <div class="flex-1">
                                <label for="photo" class="cursor-pointer">
                                    <div class="flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus-within:ring-2 focus-within:ring-yellow-500">
                                        <svg class="w-5 h-5 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                        </svg>
                                        {{ $karyawan->photo ? 'Ganti Foto' : 'Pilih Foto' }}
                                    </div>
                                    <input id="photo" name="photo" type="file" class="sr-only" accept="image/*" onchange="previewPhoto(this)">
                                </label>
                                @if($karyawan->photo)
                                    <p class="mt-2 text-xs text-gray-500">Foto saat ini: {{ basename($karyawan->photo) }}</p>
                                @endif
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-blue-700">
                                    <p>Password tidak dapat diubah melalui form ini. Gunakan tombol "Reset Password" di halaman detail karyawan untuk mengubah password.</p>
                                </div>
                            </div>
                        </div>
                    </div>
