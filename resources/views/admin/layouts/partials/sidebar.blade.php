...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-4 py-4 overflow-y-auto sidebar-scroll">
            ...
            <!-- Jadwal Management Section - Semua role tapi dengan akses berbeda -->
            <div class="mb-2">
                <div class="flex items-center px-4 py-2 mb-3">
                ...
                <div class="space-y-2">
                    ...
                    <!-- Calendar View - Semua role bisa akses -->
                    <a href="{{ route('admin.jadwal.calendar') }}"
                        ...
                        <span class="flex-1 truncate text-gray-700 group-hover:text-indigo-700">Calendar Jadwal</span>
                        @if($isCoordinator)
                        <span
                            class="nav-badge bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded-full shadow-sm">My Dept</span>
                        @else
                        <span
                            class="nav-badge bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded-full shadow-sm">All</span>
                        @endif
                    </a>
                </div>
            </div>

            <!-- Monitoring Section - HANYA ADMIN -->
            @if($isAdmin)
            <div class="mb-6">
                <div class="flex items-center px-4 py-2 mb-3">
                    <div
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.absen.index') }}"
                        class="nav-item {{ request()->routeIs('admin.absen.*') && !request()->routeIs('admin.absen.report') && !request()->routeIs('admin.absen.daily-report') ? 'active' : '' }} flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-300 group hover:scale-105"
                        @click="window.innerWidth < 1024 && (sidebarOpen = false)">
                        <div
...
<a href="{{ route('admin.tunjangan-karyawan.index') }}"
                        class="nav-item {{ request()->routeIs('admin.tunjangan-karyawan.index') || request()->routeIs('admin.tunjangan-karyawan.show') || request()->routeIs('admin.tunjangan-karyawan.edit') ? 'active' : '' }} flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-300 group hover:scale-105"
                        @click="window.innerWidth < 1024 && (sidebarOpen = false)">
                        <div
                            class="flex items-center justify-center w-8 h-8 mr-3 rounded-lg nav-icon bg-gradient-to-br from-indigo-400 to-purple-500 text-white shadow-md">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="flex-1 truncate text-gray-700 group-hover:text-indigo-700">Tunjangan Karyawan</span>
                        <div class="flex items-center">
                            @php
                                $pendingTunjangan = \App\Models\TunjanganKaryawan::where('status', 'requested')->count();
                            @endphp
                    ...
                    <a href="{{ route('admin.tunjangan-karyawan.report') }}"
                        ...
                        <span class="flex-1 truncate text-gray-700 group-hover:text-violet-700">Laporan Tunjangan</span>
                        <span
            ...
            <div class="mb-4 p-3 bg-white/70 backdrop-blur-sm rounded-2xl border border-gray-200/50 shadow-lg">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-500 truncate font-medium capitalize">{{ auth()->user()->role }}</p>
                    </div>
