...
@section('title', 'Edit Detail Nominal')
@section('breadcrumb', 'Edit Detail Nominal')
@section('page_title', 'Edit Detail Nominal Tunjangan')

@section('page_actions')
<div class="flex gap-2">
    <a href="{{ route('admin.tunjangan-detail.show', $tunjanganDetail->tunjangan_detail_id) }}"
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <div class="text-sm font-medium text-gray-900">{{ $tunjanganDetail->tunjanganType->code }}</div>
                    <div class="text-xs text-gray-500">ID: {{ $tunjanganDetail->tunjangan_detail_id }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="p-6">
        <form action="{{ route('admin.tunjangan-detail.update', $tunjanganDetail->tunjangan_detail_id) }}" method="POST" id="tunjanganDetailEditForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        @enderror
                        <p class="mt-1 text-sm text-gray-500">Jenis tunjangan untuk detail nominal ini</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="staff_status"
                            ...
                            <option value="pkwtt" {{ old('staff_status', $tunjanganDetail->staff_status) == 'pkwtt' ? 'selected' : '' }}>
                                pkwtt - Karyawan dalam masa pelatihan
                            </option>
                            <option value="karyawan" {{ old('staff_status', $tunjanganDetail->staff_status) == 'karyawan' ? 'selected' : '' }}>
                                Karyawan - Karyawan tetap
                            </option>
                            <option value="koordinator" {{ old('staff_status', $tunjanganDetail->staff_status) == 'koordinator' ? 'selected' : '' }}>
                                Koordinator - Level koordinator
                            </option>
                            <option value="wakil_koordinator" {{ old('staff_status', $tunjanganDetail->staff_status) == 'wakil_koordinator' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number"
                                   id="amount"
                                   name="amount"
                                   value="{{ old('amount', $tunjanganDetail->amount) }}"
                        ...
                        <div class="mt-1 text-sm text-gray-500">
                            <span>Nominal saat ini: Rp {{ number_format($tunjanganDetail->amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="date"
                               id="effective_date"
                               name="effective_date"
                               value="{{ old('effective_date', $tunjanganDetail->effective_date->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="date"
                               id="end_date"
                               name="end_date"
                               value="{{ old('end_date', $tunjanganDetail->end_date ? $tunjanganDetail->end_date->format('Y-m-d') : '') }}"
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            @if($tunjanganDetail->end_date)
                                Saat ini berakhir: {{ $tunjanganDetail->end_date->format('d M Y') }}. Kosongkan untuk berlaku selamanya.
                            @else
                                Saat ini berlaku selamanya. Isi untuk membatasi masa berlaku.
                            @endif
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="checkbox"
                                   ...
                                   name="is_active"
                                   value="1"
                                   {{ old('is_active', $tunjanganDetail->is_active) ? 'checked' : '' }}
                    ...
                    <!-- Preview Card -->
                    <div id="preview-card" class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                        <div class="flex items-center space-x-3">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <!-- Current Status -->
                <div class="bg-{{ $tunjanganDetail->is_active ? 'green' : 'gray' }}-50 border border-{{ $tunjanganDetail->is_active ? 'green' : 'gray' }}-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-{{ $tunjanganDetail->is_active ? 'green' : 'gray' }}-800">
                                Status Saat Ini: {{ $tunjanganDetail->is_active ? 'Aktif' : 'Tidak Aktif' }}
                            </h3>
                            <div class="mt-1 text-sm text-{{ $tunjanganDetail->is_active ? 'green' : 'gray' }}-700">
                                Dibuat: {{ $tunjanganDetail->created_at->format('d M Y H:i') }}
                                @if($tunjanganDetail->updated_at != $tunjanganDetail->created_at)
                                    <br>Diubah: {{ $tunjanganDetail->updated_at->format('d M Y H:i') }}
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-blue-800">Periode Berlaku</h3>
                            <div class="mt-1 text-sm text-blue-700">
                                Mulai: {{ $tunjanganDetail->effective_date->format('d M Y') }}
                                <br>
                                @if($tunjanganDetail->end_date)
                                    Berakhir: {{ $tunjanganDetail->end_date->format('d M Y') }}
                                @else
                                    Berlaku selamanya
                                @endif
                            </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-amber-700">
                            <p>Perubahan pada jenis tunjangan atau staff status akan divalidasi untuk mencegah overlap periode dengan detail nominal lain yang aktif. Pastikan tidak ada konflik dengan detail nominal yang sudah ada.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="existing-details" class="hidden">
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium text-yellow-800">Detail Nominal Lain yang Ada</h3>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                ...
                <button type="button"
                        ...
                        class="px-4 py-2 text-sm font-medium text-yellow-700 bg-yellow-50 border border-yellow-300 rounded-lg hover:bg-yellow-100 transition-colors">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Form validation
    const form = document.getElementById('tunjanganDetailEditForm');
...
function checkExistingDetails() {
    ...
    const currentDetailId = '{{ $tunjanganDetail->tunjangan_detail_id }}';
        ...
        const existingData = [
            {
                ...
                effective_date: '{{ $tunjanganDetail->effective_date->format('Y-m-d') }}',
                is_active: {{ $tunjanganDetail->is_active ? 'true' : 'false' }},
                is_current: true
            }
        ];

        if (existingData.length > 0) {
            let html = '<p class="mb-2">Detail nominal untuk jenis tunjangan ini:</p><ul class="list-disc pl-5 space-y-1">';
            existingData.forEach(detail => {
                ...
                const currentBadge = detail.is_current ? '<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">Sedang Diedit</span>' : '';
                html += `<li>${statusText.charAt(0).toUpperCase() + statusText.slice(1)}: Rp ${new Intl.NumberFormat('id-ID').format(detail.amount)} (berlaku ${detail.effective_date}) ${statusBadge} ${currentBadge}</li>`;
            });
            html += '</ul>';
            contentDiv.innerHTML = html;
        } else {
            contentDiv.innerHTML = '<p>Belum ada detail nominal lain untuk jenis tunjangan ini.</p>';
        }
    }, 1500);
}
</script>
