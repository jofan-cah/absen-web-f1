...
<div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
    ...
    <div class="flex flex-col sm:flex-row gap-3 flex-1">
        <div class="flex gap-2">
            <select id="month-selector" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <select id="year-selector" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                @for($y = now()->year - 2; $y <= now()->year + 2; $y++)
                    <option value="{{ $y }}" {{ $year == $y ? 'selected' : '' }}>{{ $y }}</option>
                @endfor
            </select>
            <button onclick="filterByPeriod()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <!-- Quick Navigation -->
        <div class="flex gap-2">
            <button onclick="navigateMonth(-1)" class="px-3 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </button>
            <button onclick="goToToday()" class="px-3 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors text-sm">
                Hari Ini
            </button>
            <button onclick="navigateMonth(1)" class="px-3 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Actions -->
    <div class="flex gap-2">
        <a href="{{ route('admin.jadwal.calendar') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>
            Calendar View
        </a>
        <button onclick="exportJadwal()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
    <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
        <div class="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <!-- Department Filter -->
            <select id="department-filter" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <!-- Shift Filter -->
            <select id="shift-filter" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <!-- Status Filter -->
            <select id="status-filter" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <span class="text-sm text-gray-500">Total: <span id="total-records">{{ $jadwals->count() }}</span> jadwal</span>
                ...
                <div class="flex border border-gray-300 rounded-lg overflow-hidden">
                    ...
                    <button onclick="setView('timeline')" id="timeline-view-btn" class="px-3 py-1 text-sm bg-gray-100 text-gray-600 hover:bg-gray-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h2m0-12h4m-4 0a2 2 0 002-2V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2zm4 0h6a2 2 0 012 2v10a2 2 0 01-2 2h-6"/>
                        </svg>
                    </button>
                </div>
            </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onclick="sortBy('date')">
                        Tanggal
            ...
            <tbody class="bg-white divide-y divide-gray-200" id="jadwal-table-body">
                ...
                <tr class="hover:bg-gray-50 transition-colors jadwal-row"
                    ...
                    data-shift="{{ $jadwal->shift_id }}"
                    data-status="{{ $jadwal->is_active ? '1' : '0' }}"
                    data-search="{{ strtolower($jadwal->karyawan->name . ' ' . $jadwal->shift->name) }}">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <input type="checkbox" name="selected_jadwals[]" value="{{ $jadwal->jadwal_id }}" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center">
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-gray-500">
                                    {{ $jadwal->karyawan->employee_id }} • {{ $jadwal->karyawan->department->name ?? '-' }}
                                </div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="h-8 w-8 rounded-lg bg-gradient-to-br {{ $jadwal->shift->is_overnight ? 'from-purple-500 to-indigo-600' : 'from-green-500 to-emerald-600' }} flex items-center justify-center mr-3">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-mono text-gray-900">
                            {{ \Carbon\Carbon::parse($jadwal->shift->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($jadwal->shift->end_time)->format('H:i') }}
                        </div>
                        @if($jadwal->shift->is_overnight)
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button onclick="deleteJadwal('{{ $jadwal->jadwal_id }}', '{{ $jadwal->karyawan->name }} - {{ $jadwal->date->format('d M Y') }}')"
                ...
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-500 mb-4">Belum ada jadwal untuk periode {{ DateTime::createFromFormat('!m', $month)->format('F') }} {{ $year }}</p>
                            <a href="{{ route('admin.jadwal.calendar') }}" class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
                                Kelola Jadwal
                            </a>
                        </div>
                    </td>
    ...
    <div id="timeline-view" class="hidden p-6">
        <div class="space-y-6" id="timeline-container">
            @php
                $groupedJadwals = $jadwals->groupBy(function($jadwal) {
                ...
                <div class="timeline-day" data-date="{{ $date }}">
                    <div class="flex items-center mb-4">
                        ...
                        <div class="flex-shrink-0 w-3 h-3 bg-blue-600 rounded-full relative">
                            <div class="absolute top-3 left-1/2 transform -translate-x-0.5 w-0.5 h-full bg-gray-200"></div>
                        </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">
                                                {{ $jadwal->shift->name }} • {{ $jadwal->karyawan->department->name ?? '-' }}
                                            </p>
                                        </div>
        ...
        <div class="mt-3">
            ...
            <form id="edit-form">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="edit-karyawan-name" readonly class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal</label>
                    <input type="text" id="edit-date" readonly class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                </div>

                <div class="mb-4">
                    <label for="edit-shift-id" class="block text-sm font-medium text-gray-700 mb-2">Shift</label>
                    <select id="edit-shift-id" name="shift_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="edit-notes" name="notes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"></textarea>
                </div>

                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeEditModal()" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
                        Batal
                    </button>
...
function navigateMonth(direction) {
    ...
    window.location.href = `{{ route('admin.jadwal.index') }}?month=${newMonth.toString().padStart(2, '0')}&year=${newYear}`;
}
