...
@section('content')
        ...
        <div class="mb-6 rounded-lg bg-yellow-50 border-2 border-yellow-200 p-6">
            <div class="flex items-start">
                ...
                <div class="ml-4 flex-1">
                    <h3 class="text-lg font-semibold text-yellow-900 mb-2">⚠️ Approval Langsung</h3>
                    <p class="text-yellow-800 mb-3">
                        Koordinator <strong>{{ $ijin->coordinator->name ?? '-' }}</strong> belum review ijin ini.
                    </p>
                    <p class="text-yellow-800 mb-3">
                        Jika Anda approve, sistem akan:
                    </p>
                    <ul class="list-disc list-inside text-yellow-800 space-y-1 mb-4">
                        <li>Langsung approve ijin ini (FINAL)</li>
                        <li>Set koordinator status = approved otomatis</li>
                        <li>Mencatat bahwa admin melakukan bypass</li>
                    </ul>
                    <div class="bg-yellow-100 rounded-lg p-3 border border-yellow-300">
                        <p class="text-sm text-yellow-900">
                            <strong>Catatan:</strong> Keputusan Anda sebagai admin adalah final!
                        </p>
                    </div>
                </div>
            </div>
    ...
    @else
        <div class="mb-6 rounded-lg bg-green-50 border-2 border-green-200 p-4">
            <div class="flex items-start">
                ...
                <div class="flex-1">
                    <p class="text-sm text-green-800">
                        ✅ Koordinator sudah approve. Anda dapat melakukan approval final.
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex items-center">
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-lg bg-pink-100 text-pink-800 font-medium">
                                {{ $ijin->ijinType->name }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    @if ($ijin->coordinator_reviewed_at)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="text-sm text-gray-900">{{ $ijin->coordinator_reviewed_at->format('d M Y H:i') }}</span>
                        </div>
                    @endif
                ...
                <form id="reviewForm" method="POST" action="{{ route('admin.ijin.admin-review', $ijin->ijin_id) }}">
                    ...
                    @if ($needsBypass)
                        <div class="mb-6 p-4 bg-yellow-50 border-2 border-yellow-300 rounded-lg">
                            <div class="flex items-start">
                                <input type="checkbox" name="bypass_coordinator" id="bypass_coordinator" value="1"
                                    class="mt-1 h-5 w-5 text-yellow-600 focus:ring-yellow-500 border-gray-300 rounded">
                                <label for="bypass_coordinator" class="ml-3 text-sm">
                                    <span class="font-semibold text-yellow-900">Bypass Koordinator</span>
                                    <p class="text-yellow-800 mt-1">Saya memahami dan ingin melewati approval koordinator.
                                        Tindakan ini akan dicatat dalam sistem.</p>
                                </label>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="note" id="note" rows="4"
                            ...
                            placeholder="Tambahkan catatan review Anda...">{{ old('note') }}</textarea>
                        <p class="text-xs text-gray-500 mt-1">Catatan akan dilihat oleh karyawan dan tersimpan dalam
                            histori</p>
                    </div>
                    ...
                    <div class="space-y-3">
                        <button type="button" onclick="submitReview('approve')"
                            class="w-full inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm font-semibold rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        ...
                        <button type="button" onclick="submitReview('reject')"
                            class="w-full inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-red-500 to-rose-600 text-white text-sm font-semibold rounded-lg hover:from-red-600 hover:to-rose-700 transition-all duration-200 shadow-md hover:shadow-lg">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a href="{{ route('admin.ijin.admin-pending') }}"
                            class="w-full inline-flex items-center justify-center px-6 py-3 bg-gray-200 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 transition-colors">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                            </svg>
        ...
        function submitReview(action) {
            ...
            @if ($needsBypass)
                if (action === 'approve' && !bypassCheckbox.checked) {
                    if (!confirm(
                            'Koordinator belum mereview ijin ini. Apakah Anda yakin ingin approve tanpa centang "Bypass Koordinator"?\n\nJika ya, Anda harus mencentang checkbox bypass terlebih dahulu.'
                            )) {
                        return;
                    }
            ...
            if (action === 'approve') {
                @if ($needsBypass)
                    message =
                        'Anda akan APPROVE ijin ini dengan BYPASS koordinator.\n\nTindakan ini akan dicatat dalam log audit.\n\nLanjutkan?';
                @else
