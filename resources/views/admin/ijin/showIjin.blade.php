                    ...
                    @if ($ijin->hasPhoto())
                        <div class="md:col-span-2">
                            ...
                            <div class="relative group cursor-pointer"
                                onclick="openPhotoModal('{{ $ijin->photo_url }}')">
                                <img src="{{ $ijin->photo_url }}" alt="Lampiran Ijin"
                                ...
                                <div
                                    ...
                                    <div
                                        class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white rounded-full p-3 shadow-lg">
            ...
            @if ($ijin->status === 'pending')
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        {{-- UNTUK KOORDINATOR - TOMBOL ACC LANGSUNG --}}
                        @if (auth()->user()->role === 'coordinator' &&
                                $ijin->coordinator_id === auth()->user()->user_id &&
                                $ijin->coordinator_status === 'pending')
                            {{-- Tombol ke halaman review (yang lama) --}}
                            <a href="{{ route('admin.ijin.coordinator-review-form', $ijin->ijin_id) }}"
                                class="w-full inline-flex items-center justify-center px-4 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-600 text-white text-sm font-semibold rounded-lg hover:from-yellow-600 hover:to-orange-700 transition-all duration-200 shadow-md hover:shadow-lg">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            {{-- ATAU BISA LANGSUNG ACC DI SINI --}}
                            <div class="border-t border-gray-200 pt-3 mt-3">
                                <p class="text-xs text-gray-600 mb-3 text-center">Atau langsung approve/reject:</p>

                                <form id="coordinatorReviewForm" method="POST"
                                    action="{{ route('admin.ijin.coordinator-review', $ijin->ijin_id) }}">
                                    @csrf
                                    ...
                                    {{-- Tombol Approve --}}
                                    <button type="button" onclick="submitCoordinatorReview('approve')"
                                        class="w-full inline-flex items-center justify-center px-4 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm font-semibold rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg mb-2">
                                    ...
                                    {{-- Tombol Reject --}}
                                    <button type="button" onclick="submitCoordinatorReview('reject')"
                                        class="w-full inline-flex items-center justify-center px-4 py-2.5 bg-gradient-to-r from-red-500 to-rose-600 text-white text-sm font-semibold rounded-lg hover:from-red-600 hover:to-rose-700 transition-all duration-200 shadow-md hover:shadow-lg">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if (auth()->user()->role === 'admin')
                            <a href="{{ route('admin.ijin.admin-review-form', $ijin->ijin_id) }}"
                                ...
                                Review sebagai Admin
                                @if ($ijin->coordinator_status === 'pending')
                                    <span class="ml-2 text-xs">(Bypass)</span>
                                @endif
                            </a>
                        @endif
                    </div>
...
@push('scripts')
<script>
function submitCoordinatorReview(action) {
    const form = document.getElementById('coordinatorReviewForm');
