...
@section('page_title', 'Laporan Absensi Bulanan - ' . DateTime::createFromFormat('!m', $month)->format('F') . ' ' .
    $year)
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="text-gray-600">Laporan Absensi
                    Bulanan{{ request('department_id') ? ' - ' . $departments->firstWhere('department_id', request('department_id'))->name : '' }}
                </p>
            </div>
            ...
            <div class="mt-4 lg:mt-0 flex flex-wrap gap-4">
                @php
                    ...
                    $totalHadir =
                        array_sum(array_column($reportData, 'hadir')) +
                        array_sum(array_column($reportData, 'terlambat'));
            ...
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        {{ array_sum(array_column($reportData, 'terlambat')) }}</p>
                </div>
            </div>
            <div class="mt-4">
                @php
                    ...
                    $avgTerlambat =
                        array_sum(array_column($reportData, 'terlambat')) > 0
                            ? round($totalTerlambatMenit / array_sum(array_column($reportData, 'terlambat')), 1)
                            : 0;
                @endphp
                <div class="flex items-center text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        {{ array_sum(array_column($reportData, 'tidak_hadir')) }}</p>
                </div>
            </div>
            <div class="mt-4">
                @php
                    $absentPercentage =
                        $totalJadwal > 0
                            ? round((array_sum(array_column($reportData, 'tidak_hadir')) / $totalJadwal) * 100, 1)
                            : 0;
                @endphp
                <div class="flex items-center text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        {{ number_format(array_sum(array_column($reportData, 'total_jam_kerja')), 0) }}</p>
                </div>
            </div>
            <div class="mt-4">
                @php
                    $avgJamKerja =
                        $totalKaryawan > 0
                            ? round(array_sum(array_column($reportData, 'total_jam_kerja')) / $totalKaryawan, 1)
                            : 0;
                @endphp
                <div class="flex items-center text-sm">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Ringkasan kehadiran karyawan bulan
                        {{ DateTime::createFromFormat('!m', $month)->format('F') }} {{ $year }}</p>
                </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total
                            Jadwal</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Hadir
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Terlambat</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Pulang
                            Cepat</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Tidak
                            Hadir</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total
                            Jam</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">%
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($reportData as $index => $data)
                        <tr class="hover:bg-gray-50 transition-colors report-row"
                            ...
                            data-late="{{ $data['terlambat'] }}" data-absent="{{ $data['tidak_hadir'] }}"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900">{{ $data['karyawan']->full_name }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    {{ $data['karyawan']->department->name ?? 'No Department' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @php
                                    $attendanceRate =
                                        $data['total_jadwal'] > 0
                                            ? round(
                                                (($data['hadir'] + $data['terlambat']) / $data['total_jadwal']) * 100,
                                                1,
                                            )
                                            : 0;
                                @endphp
                                <div class="flex items-center justify-center">
                                    <span
                                        class="text-sm font-medium {{ $attendanceRate >= 90 ? 'text-green-600' : ($attendanceRate >= 75 ? 'text-yellow-600' : 'text-red-600') }}">
                                        {{ $attendanceRate }}%
                                    </span>
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center no-print">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $performanceColor }}">
                                    {{ $performance }}
                                </span>
                            </td>
                        </tr>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div class="text-center">
                        ...
                        <span
                            class="font-semibold text-gray-900">{{ array_sum(array_column($reportData, 'total_jadwal')) }}</span>
                    </div>
                    <div class="text-center">
                        <span class="font-medium text-gray-600">Total Kehadiran:</span>
                        <span
                            class="font-semibold text-gray-900">{{ array_sum(array_column($reportData, 'hadir')) + array_sum(array_column($reportData, 'terlambat')) }}</span>
                    </div>
                    <div class="text-center">
