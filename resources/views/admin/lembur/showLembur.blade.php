...
<div class="flex gap-2">
    ...
    @elseif($lembur->koordinator_status == 'approved' && $lembur->status == 'submitted')
        <div class="px-4 py-2 bg-yellow-100 text-yellow-700 rounded-lg flex items-center gap-2">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2 sm:col-span-1">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-2xl font-bold text-purple-600">{{ number_format($lembur->total_jam, 1) }}</span>
                ...
                <div class="col-span-2">
                    ...
                    <div class="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-4">
                        @php
                            ...
                            $amountPerUnit = in_array($staffStatus, ['karyawan', 'koordinator', 'wakil_koordinator']) ? 20000 : 15000;
                            $totalAmount = $amountPerUnit * $quantity;
                        @endphp
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-purple-600 font-medium">{{ $quantity }}x Uang Makan</p>
                                <p class="text-xs text-purple-500">Rp {{ number_format($amountPerUnit, 0, ',', '.') }} × {{ $quantity }}</p>
                            </div>
                            <p class="text-xl font-bold text-purple-700">Rp {{ number_format($totalAmount, 0, ',', '.') }}</p>
                        </div>
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                    ...
                    <a href="{{ route('admin.tunjangan-karyawan.show', $lembur->tunjanganKaryawan->tunjangan_karyawan_id) }}"
                       class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="text-sm text-gray-500">
                    <p>Status: <span class="font-medium text-gray-900">{{ ucfirst($lembur->tunjanganKaryawan->status) }}</span></p>
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="flex items-center mb-4">
                <div class="flex-shrink-0 h-16 w-16">
                    <div class="h-16 w-16 rounded-full bg-gradient-to-br from-purple-400 to-indigo-500 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
        ...
        <div class="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl shadow-sm border border-purple-200 p-6">
            <h3 class="text-lg font-semibold text-purple-900 mb-3">⚡ Quick Actions</h3>
            <div class="space-y-2">
...
function approveLembur() {
    const notes = prompt('💬 Catatan persetujuan (opsional):\n\nSetelah Anda approve, lembur akan diteruskan ke Admin untuk final approval dan generate tunjangan.');
    if (notes === null) return; // User cancelled

    if (!confirm('✅ Approve lembur ini?\n\n⚠️ Pastikan:\n- Jam lembur sudah sesuai\n- Bukti foto valid\n- Deskripsi pekerjaan jelas\n\nSetelah approve, akan diteruskan ke Admin.')) return;
...
function rejectLembur() {
    ...
    .catch(error => {
        hideLoading();
        showErrorModal('Terjadi kesalahan saat reject');
        console.error('Error:', error);
    });
}
    ...
    const loadingHTML = `
        <div id="loadingOverlay" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
            <div class="bg-white rounded-lg p-6 flex flex-col items-center">
                <svg class="animate-spin h-10 w-10 text-purple-600 mb-3" fill="none" viewBox="0 0 24 24">
